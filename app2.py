...
# District coordinates (27 districts)
district_coords = {
    "Ahmadnagar": (19.0948, 74.7476), "Akola": (20.6976, 77.0081), "Amravati": (20.9374, 77.7796),
    "Aurangabad": (19.8762, 75.3433), "Bid": (18.9873, 75.7570), "Buldana": (20.5293, 76.1804),
    "Dhule": (20.9042, 74.7749), "Hingoli": (19.7170, 77.1482), "Jalgaon": (21.0077, 75.5626),
    "Jalna": (19.8410, 75.8864), "Kolhapur": (16.7050, 74.2433), "Latur": (18.4088, 76.5604),
    "Nanded": (19.1383, 77.3210), "Nandurbar": (21.3735, 74.2390), "Nashik": (19.9975, 73.7898),
    "Osmanabad": (18.1810, 76.0419), "Palghar": (19.6967, 72.7656), "Parbhani": (19.2707, 76.7600),
    "Pune": (18.5204, 73.8567), "Raigarh": (18.6447, 73.3000), "Ratnagiri": (16.9902, 73.3120),
    "Sangli": (16.8524, 74.5815), "Satara": (17.6805, 74.0183), "Sindhudurg": (16.1222, 73.6810),
    "Solapur": (17.6599, 75.9064), "Thane": (19.2183, 72.9781), "Washim": (20.1112, 77.1333)
}
...
crop_info = {
    "Pomegranate": {
        "description": "A fruit-bearing deciduous shrub, widely cultivated in tropical and subtropical regions.",
        "image": "/static/img/pomegranate.jpg",
        "optimal_conditions": "Well-drained soil, full sun exposure, and moderate rainfall.",
        "growth_cycle": "10-12 months from planting to harvest.",
        "avg_yield": "12-16 tonnes per hectare"
    },
    "Mango": {
        "description": "A tropical tree cultivated for its edible fruit, known as the 'King of Fruits'.",
        "image": "/static/img/mango.jpg",
        "optimal_conditions": "Deep, well-drained soil, warm climate, and protection from strong winds.",
        "growth_cycle": "3-6 months from flowering to harvest.",
...
def calculate_payout(district, season, year, crop, area):
    ...
    sum_insured = float(sum_insured_model.predict(si_input).flatten()[0])
    estimated_payout = round(avg_percent * sum_insured * area, 2)
...
def index():
    # Define available crops by season
    ...
    if request.method == "POST":
        ...
        # Keep them for re-render in case of error or success
        ...
        # Add extra info for template rendering
        result["district"] = district
        result["district_coords"] = district_coords[district]
